...
int main() {
    ...
    std::string vsPath = SLN_SOURCE_CODE_DIR + std::string("DiffuseColor.vs.glsl");
    std::string fsPath = SLN_SOURCE_CODE_DIR + std::string("DiffuseColor.fs.glsl");
    Shader shaderDiffuse(vsPath.c_str(), fsPath.c_str());


    std::string vsEdgePath = SLN_SOURCE_CODE_DIR + std::string("Edge.vs.glsl");
    std::string fsEdgePath = SLN_SOURCE_CODE_DIR + std::string("Edge.fs.glsl");
    Shader shaderEdge(vsEdgePath.c_str(), fsEdgePath.c_str());
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        shaderDiffuse.setMat4("model", glm::mat4(1.0f));
        glDrawArrays(GL_TRIANGLES, 0, 6);
        glBindVertexArray(0);

        // 1、 First pass diffuse color
